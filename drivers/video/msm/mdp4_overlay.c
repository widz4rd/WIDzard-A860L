...
#ifdef PANTECH_LCD_BUG_FIX_QCPATCH_FOR_MDP_COMP_WITH_AOT
...
#ifdef CONFIG_PANTECH_LCD_FIX_UNDERRUN_WITH_3LAYER 
        // Sum except for default pipe.
        if(pipe->pipe_ndx != 1)
            total_size += pipe->dst_w * pipe->dst_h;
#endif

#ifdef PANTECH_LCD_BUG_FIX_MDP_BANDWIDTH_REQUEST
		//pr_info("[LIVED] cnt=%d, pipe->flags=%d, pipe->pipe_type=%d\n", cnt, pipe->flags, pipe->pipe_type);
		if (!(pipe->flags & MDP_BACKEND_COMPOSITION) && pipe->pipe_type == OVERLAY_TYPE_RGB) {
			fb_layer++;
		}
#endif
...
#ifdef CONFIG_PANTECH_LCD_FIX_UNDERRUN_WITH_3LAYER
    // 3 layer bypass mode -> cnt 4 = 3(bypass)+1(default pipe)
    if(cnt >= 4) {
        //printk("choisy total_size %d %d\n", total_size, 1920*1080 + 1920*1080);
        if(total_size > 1920*1080 + 1920*1080)
            perf_req->mdp_bw = OVERLAY_PERF_LEVEL0;
    }
#endif

#ifdef PANTECH_LCD_BUG_FIX_MDP_BANDWIDTH_REQUEST
	if (cnt == 1 && fb_layer == 1) {
		perf_req->mdp_bw = OVERLAY_PERF_LEVEL4;
	}
#endif
...
#ifdef CONFIG_F_SKYDISP_QCBUGFIX_BLOCK_BLT_MODE_AND_TUNE_ROTATER_RATIO
