...
#ifdef PANTECH_LCD_FIX_ABOUT_HALT_WHEN_LCD_ONOFF_DURING_WFD
	mdp_clk_ctrl(1);
#else
...
#ifdef CONFIG_PANTECH_LCD_FIX_LOCKUP_WHEN_CONNECT_LGE_TV_WITH_WFD				
		wait_for_completion_interruptible_timeout(&mfd->dma->comp,
		...
		struct mdp4_overlay_pipe *pipe)
{
...
#ifdef CONFIG_F_SKYDISP_FIX_WFD_DATA_ABORT_BECAUSE_OF_MUTEX
	// Wait 1000ms if mutex is not initialized.
	{
		int i;
		for(i=0; i<10; i++)
		{
			if(mfd->unregister_mutex.wait_list.next != NULL && mfd->writeback_mutex.wait_list.next != NULL)
				break;

            printk(KERN_ERR "%s() %p %p\n", __func__, mfd->unregister_mutex.wait_list.next, mfd->writeback_mutex.wait_list.next);
			printk(KERN_ERR "mutex is not initialized, retry every 100ms\n");
...
#ifdef CONFIG_PANTECH_LCD_FIX_LOCKUP_WHEN_CONNECT_LGE_TV_WITH_WFD	
	{
		...
		if (data == 0) {
			pr_err(">>>>>> MUX is not set properly : %x\n", data);
			if (hdmi_prim_display)
				data = 0x01;
			else
				data = 0x02;
