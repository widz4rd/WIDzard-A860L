...
static int msmsdcc_bam_dml_reset_and_restore(struct msmsdcc_host *host)
{
	...
	// <20130311> sim.jungsun. QCT Patch.
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	//msmsdcc_trace_init(host); //<20130109> p14986 sim,jungsun. QCT Patch
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	struct msmsdcc_host *host = mmc_priv(mmc);
	unsigned long flags;
	//int rc = 0; //<20130109> p14986 sim,jungsun. QCT Patch
	...
	if (mmc) {
		...
		mmc_resume_host(mmc);
		//rc = mmc_resume_host(mmc); //<20130109> p14986 sim,jungsun. QCT Patch
